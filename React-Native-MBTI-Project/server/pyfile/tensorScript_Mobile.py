...
def predict_mbti_kakaotalk(filename):
    ...
    user_messages = {}
    current_user = None
    pattern = r'^(\d{4}년\s\d{1,2}월\s\d{1,2}일\s(?:오전|오후)\s\d{1,2}:\d{2}),\s([^:\n]+):\s(.*)$'
    for line in lines[2:]:
        ...
        if match:
            ...
            # Handle the "받은 명단에~" part as a separate message
    ...
    for user, messages in user_messages.items():
        ...
        top_k = sorted(prediction.items(), key=lambda x: x[1], reverse=True)[:7]  # Adjust the number of top results
